...
function start_net(){
	...
	sudo ip r a 192.168.4.0/24 dev vhost0
	sudo iptables -t nat -A POSTROUTING -s 192.168.4.0/24 ! -o vhost0 -j MASQUERADE
}
...
function start_net_tun(){
	...
	sudo ip r a 192.168.4.0/24 dev $DPSW_NETA_IF
	sudo iptables -t nat -A POSTROUTING -s 192.168.4.0/24 ! -o $DPSW_NETA_IF -j MASQUERADE
...
function stop_net(){
	...
	sudo iptables -t nat -D POSTROUTING -s 192.168.4.0/24 ! -o $DPSW_NETA_IF -j MASQUERADE
	sudo ip r del 192.168.4.0/24 dev $DPSW_NETA_IF
